...
const oldHeader = "function renderChatHeader(){const c=state.currentChat;if(!c)return;const p=profileBy(c.profile_id);$('chatName').textContent=c.contact_name||c.wa_jid;$('chatAvatar').textContent=initials(c.contact_name||c.wa_jid);$('chatMeta').textContent=(p?.name||'Profil')+' · '+(c.state==='HOT'?'HOT':c.state==='HUMAN_ACTIVE'?'von dir übernommen':'KI aktiv');$('takeBtn').style.display=c.state==='HOT'?'inline-block':'none';$('returnBtn').style.display=c.state==='HUMAN_ACTIVE'?'inline-block':'none';$('hotStrip').classList.toggle('show',c.state==='HOT');$('hotReason').textContent=c.hot_reason?'🔥 '+c.hot_reason:'🔥 HOT Lead wartet auf Übernahme'}";
const newHeader = oldHeader.slice(0, -1) + ";const canWrite=c.state==='HUMAN_ACTIVE';$('composer').disabled=!canWrite;$('composer').placeholder=canWrite?'Nachricht eingeben':'Chat zuerst übernehmen';document.querySelector('.sendBtn').disabled=!canWrite}";
html = replaceRequired(html, oldHeader, newHeader, 'composer ownership state');
html = replaceRequired(
  html,
...
const newFill = oldFill.slice(0, -1) + ";$('pModel').value=p.llm_model_override||'';$('pTemp').value=p.temperature??.35;$('pVoiceName').value=p.voice_name||'alloy';$('pMedia').value=Array.isArray(p.media)?p.media.join('\\n'):''}";
html = replaceRequired(html, oldFill, newFill, 'advanced profile fill');
html = replaceRequired(
  html,
...
const oldUnlink = "async function unlinkProfile(){if(!state.editingProfile||!confirm('WhatsApp-Verknüpfung lösen?'))return;await api('/api/profiles/'+state.editingProfile+'/unlink',{method:'POST',body:'{}'});toast('Verknüpfung gelöst');await load();editProfile(state.editingProfile)}";
...
html = replaceRequired(html, oldUnlink, oldUnlink + deleteFn, 'profile delete action');
