...
test('duplicate inbound WhatsApp IDs are rejected before persistence or AI run', () => {
  ...
  const persistence = handler.indexOf('persistInboundMessage(');
  const aiRun = handler.indexOf('qualifyLead(');
  assert.ok(duplicateCheck >= 0, 'duplicate message check missing');
  assert.ok(persistence > duplicateCheck, 'duplicate check must happen before inbound persistence');
